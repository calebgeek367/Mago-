...
export const BackgroundCells = ({ children, className }: BackgroundCellsProps) => {
  return (
    <div className={cn("relative min-h-screen h-auto flex justify-center overflow-hidden bg-slate-950", className)}>
      <BackgroundCellCore />
      {children && (
        <div className="relative z-50 pointer-events-auto select-auto flex flex-col justify-center w-full">
          {children}
        </div>
      )}
    </div>
...
const Pattern = ({ className, cellClassName }: PatternProps) => {
  // Increased grid density slightly to cover more area if needed
  ...
  return (
    <div className={cn("flex flex-row relative z-30 justify-center", className)}>
